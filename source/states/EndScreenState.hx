...
class EndScreenState extends FlxState {
    ...
    override public function create() {
		...
		FlxTween.num(0, scoreObj.score, 1, {startDelay: 0.5, ease: FlxEase.expoOut},                                        
		n -> {
		...
		for (i in 0...6)
		{
			...
			FlxTween.tween(judgementTxt, {x: FlxG.width / 2 - 300, alpha: 1}, 1, {ease: FlxEase.expoOut, startDelay: 2.25 + i * 0.3});
		}
		...
		FlxTween.tween(rankTxt, {'scale.x': 1, 'scale.y': 1, alpha: 1}, 1,
			{ease: n -> Math.pow(n, 10 / 3), startDelay: 3, onComplete: t -> FlxG.camera.flash()});
